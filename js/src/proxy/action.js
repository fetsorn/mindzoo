 ...
 * @returns {object}
 */
export async function changeMind(fs, pathname, searchString) {
  ...
  const { mind: mindRecord } = shouldClone
    ? await clone(fs, undefined, remoteUrl, token)
    : await find(fs, mind, undefined);
  console.log("[proxy] changeMind: mindRecord", mindRecord);

  const schema = await readSchema(fs, mindRecord.mind);
