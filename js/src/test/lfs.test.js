...
import { gitinit, commit, setOrigin } from "../git.js";
...
describe("createLFS", () => {
  ...
  test("throws if no mind", async () => {
    await expect(createLFS(stub.fs, stub.mind)).rejects.toThrowError();
  });
  ...
  test("writes git config", async () => {
    ...
    await createLFS(stub.fs, stub.mind);

    // writes .gitattributes
    const gitattributes = await stub.fs.promises.readFile(
    ...
    // sets git config
    const gitconfig = await stub.fs.promises.readFile(
...
describe("addLFS", () => {
  ...
  test("throws if not lfs dir", async () => {
    await expect(
      addLFS(stub.fs, stub.dirpath, stub.filename),
  ...
  test("calls isogit-lfs", async () => {
    ...
    await addLFS(stub.fs, stub.dirpath, assetPath);
...
describe("putAsset", () => {
  ...
  test("throws if no mind", async () => {
    await expect(
      putAsset(stub.fs, stub.mind, stub.filename, stub.content),
    ).rejects.toThrowError();
  });
  ...
  test("calls io", async () => {
    ...
    await putAsset(stub.fs, stub.mind, stub.filename, stub.content);
...
describe("setAssetPath", () => {
  ...
  test("throws if no mind", async () => {
    await expect(
      setAssetPath(stub.fs, stub.mind, stub.dirpath),
    ).rejects.toThrowError();
  });
  ...
  test("setAssetPath", async () => {
    ...
    await setAssetPath(stub.fs, stub.mind, stub.dirpath);
...
describe("getAssetPath", () => {
  ...
  test("throws if no mind", async () => {
    await expect(getAssetPath(stub.fs, stub.mind)).rejects.toThrowError();
  });
  ...
  test("calls git", async () => {
    ...
    await getAssetPath(stub.fs, stub.mind, stub.dirpath);
  ...
  test("reads asset path", async () => {
    ...
    const listing = await getAssetPath(stub.fs, stub.mind, stub.dirpath);
...
describe("fetchAsset", () => {
  ...
  test("throws if no mind", async () => {
    await expect(
      fetchAsset(stub.fs, stub.mind, stub.filename),
    ).rejects.toThrowError();
  });
  ...
  test("fetches a blob from pointer", async () => {
    ...
    await stub.fs.promises.writeFile(
    ...
    const content = await fetchAsset(stub.fs, stub.mind, stub.filename);
...
describe("uploadBlobsLFS", () => {
  ...
  test("throws if no mind", async () => {
    await expect(
      uploadBlobsLFS(stub.fs, stub.mind, stub.remote, []),
    ).rejects.toThrowError();
  });
  ...
  test("uploads files from parameters", async () => {
    ...
    await uploadBlobsLFS(stub.fs, stub.mind, stub.url, stub.token, [""]);
  ...
  test("uploads files from asset path", async () => {
    ...
    await uploadBlobsLFS(stub.fs, stub.mind, stub.url, stub.token, undefined);
...
describe("uploadFile", () => {
  ...
  test("uploads a file", async () => {
    ...
    const content = await stub.fs.promises.readFile(
