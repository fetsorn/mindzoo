...
pub fn create_lfs(path: PathBuf, mind: &str) -> Result<()>
{
...
pub async fn fetch_asset(path: PathBuf, mind: &str, filename: &str) -> Result<Vec<u8>>
{
    let filename = sanitize_filename(filename)?;
    let mind_obj = Mind::new(path, mind);
...
pub async fn put_asset(path: PathBuf, mind: &str, filename: &str, buffer: Vec<u8>) -> Result<()>
{
    let filename = sanitize_filename(filename)?;
    let mind_obj = Mind::new(path, mind);
    let mind_dir = mind_obj.find_mind()?.ok_or_else(|| Error::from_message("no mind found"))?;
...
pub async fn upload_file(_path: PathBuf, _mind: &str) -> Result<Vec<serde_json::Value>>
{
    //let mind_obj = Mind::new(path, mind);
    //let mind_dir = mind_obj.find_mind()?.ok_or_else(|| Error::from_message("no mind found"))?;
...
pub async fn set_asset_path(path: PathBuf, mind: &str, asset_path: &str) -> Result<()>
{
...
pub async fn get_asset_path(path: PathBuf, mind: &str) -> Result<Option<String>>
{
    let mind_obj = Mind::new(path, mind);
    let mind_dir = mind_obj.find_mind()?.ok_or_else(|| Error::from_message("no mind found"))?;
